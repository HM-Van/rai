...
ptr<ComputeObject> skeleton2Bound(ptr<KOMO>& komo, BoundType boundType, const Skeleton& S,
  ...
  if(boundType==BD_pose)
    return make_shared<PoseBound>(komo, S, startKinematics, collisions, initnoise);

  if(boundType==BD_seq || boundType==BD_poseFromSeq)
    return make_shared<SeqBound>(komo, S, startKinematics, collisions, initnoise);

  if(boundType==BD_path)
    return make_shared<PathBound>(komo, S, startKinematics, collisions,initnoise);

  if(boundType==BD_seqPath)
    return make_shared<SeqPathBound>(komo, S, startKinematics, collisions, waypoints,initnoise);

  if(boundType==BD_seqVelPath)
    return make_shared<SeqVelPathBound>(komo, S, startKinematics, collisions, waypoints,initnoise);
...
PoseBound::PoseBound(ptr<KOMO>& komo,
                     ...
                     bool collisions, double initnoise)
